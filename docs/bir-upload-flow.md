...
### Multi-Step Form Premature Submission

- **Symptom**: When navigating the BIR form, clicking "Next" on the penultimate step (Step 5) would cause the UI to briefly flash the final step (Step 6) and then immediately submit the form. This prevented users from interacting with the final step.
- **Root Cause**: The issue was a side-effect of using `react-hook-form` within a single `<form>` element for the entire multi-step process. The form had a global `onSubmit` handler. The "Next" button's `onClick` handler would trigger a successful partial validation, which, due to the global `onSubmit`, would then incorrectly trigger the final submission logic as a chained reaction.
- **Resolution**: The bug was resolved by decoupling the navigation from the final submission in `src/features/bir/MultiStepBirForm.tsx`:
    1.  The `onSubmit` handler was removed from the `<form>` element entirely.
    2.  The "Submit All Answers" button on the final step was changed to `type="button"`, and its `onClick` handler was explicitly wrapped with `form.handleSubmit()`. This ensures the submission logic is only ever triggered by a direct click on that specific button. 