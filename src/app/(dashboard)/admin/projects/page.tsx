  ...
  const handleSaveEdit = async (formData: {
    ...
    try {
      ...
      setProjects(prevProjects => 
        prevProjects.map(p => 
          (p.id === editProject.id && p.type === editProject.type) ? { ...p, ...project } : p
        )
      );
                ...
                <div className="grid gap-4">
                  ...
                  {projectDetails.deadline && (
                    <div>
                      ...
                      <p className="text-sm">
                        {projectDetails.deadline && !isNaN(new Date(projectDetails.deadline).getTime()) 
                          ? format(new Date(projectDetails.deadline), 'MMMM d, yyyy')
                          : 'Invalid date'}
                      </p>
                    </div>
                  )}
                    ...
                    <div>
                      ...
                      {projectDetails.designer_assignment.assigned_at && (
                        <p className="text-xs text-muted-foreground">
                          Assigned: {projectDetails.designer_assignment.assigned_at && !isNaN(new Date(projectDetails.designer_assignment.assigned_at).getTime())
                            ? format(new Date(projectDetails.designer_assignment.assigned_at), 'MMMM d, yyyy')
                            : 'Invalid date'}
                        </p>
                      )}
                    </div>
          ...
          <DialogContent className="sm:max-w-[600px] bg-popover">
            ...
            <form onSubmit={(e) => {
              ...
              const formData = {
                ...
                deadline: (form.elements.namedItem('deadline') as HTMLInputElement).value || undefined
              };
