...
/**
 * @swagger
 * /api/admin/projects/logo_design/{projectId}:
 *   get:
 *     summary: Fetch detailed logo design project information (Admin)
 *     description: Retrieves detailed information for a specific logo design project, including client and assigned designer data. Requires admin privileges.
 *     tags: [Admin Projects]
 *     parameters:
 *       - in: path
 *         name: projectId
 ...
 *           type: string
 *           format: uuid
 *         description: The UUID of the logo design project.
 *     responses:
 *       200:
 ...
 *           application/json:
 *             schema:
 *               type: object # Add specific schema for LogoDesignProject + client + designer_assignment
 *       401:
...
export async function GET(request: NextRequest, { params }: Params) {
  try {
    ...
    if (projectError) {
      console.error('Error fetching logo_design_project details:', projectError);
    ...
    const { data: assignmentData, error: assignmentError } = await adminClient
      ...
      .eq('project_type', 'logo_design') // Explicitly match type
      .maybeSingle();
 ...
 * /api/admin/projects/logo_design/{projectId}:
 *   put:
 *     summary: Update a logo design project (Admin)
 *     description: Updates fields for a specific logo design project. Requires admin privileges. Validates status against allowed values.
 *     tags: [Admin Projects]
 *     parameters:
 *       - in: path
 *         name: projectId
 ...
 *           type: string
 *           format: uuid
 *         description: The UUID of the logo design project to update.
 *     requestBody:
 *       required: true
 *       content:
 *         application/json:
 ...
 *               # Add other updatable fields for logo_design_projects
 *     responses:
 *       200:
...
export async function PUT(request: NextRequest, { params }: Params) {
  try {
    ...
    // Allowed statuses for logo_design_projects (confirm this matches schema if needed)
    ...
    if (error) {
      console.error('Error updating logo_design_project:', error);
    ...
    return NextResponse.json({ message: 'Logo design project updated successfully!', project: data });
