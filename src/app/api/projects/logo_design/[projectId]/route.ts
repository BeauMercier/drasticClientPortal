...
/**
 * @swagger
 * /api/projects/logo_design/{projectId}:
 *   get:
 *     summary: Fetch detailed logo design project information (Client/Designer)
 *     description: Retrieves detailed information for a specific logo design project if the authenticated user is the owner (client) or the assigned designer.
 *     tags: [Projects]
 *     parameters:
 *       - in: path
 *         name: projectId
...
export async function GET(request: NextRequest, { params }: Params) {
  try {
    ...
    const projectType = 'logo_design'; // Hardcoded for this route file
    ...
    if (!authorized) {
      ...
      const { data: existsCheck } = await adminClientCheck.from('logo_design_projects').select('id').eq('id', projectId).maybeSingle();
      if (!existsCheck) {
    ...
    if (projectError) {
      console.error('Error fetching logo_design_project details (authed):', projectError);
